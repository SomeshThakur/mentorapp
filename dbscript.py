...
def process_marks(wb):
    ...
    with click.progressbar(_sheet,
        ...
        for row in sheet:
            ...
            if s is None:
                continue
            total = int(tmp[1]) + int(tmp[2]) + int(tmp[3]) + int(tmp[4])
            m = Mocktest(problem1=int(tmp[1]), problem2=int(tmp[2]), problem3=int(tmp[3]), problem4=int(tmp[4]),
                         totals=total, student=s)
            m.save()
